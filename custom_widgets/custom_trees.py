...
class PlotTree(tk.Frame):
    def __init__(self, parent, plot, closing, ancestor):
        ...
        treeview = ttk.Treeview(self, show='tree headings', columns=str(row_no), style="Mystyle.Treeview", name='plot')
        ...
        treeview.configure(height=21, yscrollcommand=verscrlbar.set)
        ...
        treeview.bind('<<TreeviewSelect>>', lambda g: sel_item_action(g, label1, self.fast_close, ancestor, 0))
        treeview.bind('<Double-1>', lambda g: sel_item_action(g, label1, self.fast_close, ancestor, 1))
